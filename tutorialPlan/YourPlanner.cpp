...
YourPlanner::solve()
{
  ...
  while ((::std::chrono::steady_clock::now() - this->time) < this->duration)
  {
    ...
    for (::std::size_t j = 0; j < 2; ++j)
    {
      ...
      // uniform sampling:
      // // yourplanner override this function, so "this" will be YourPlanner instead of RrtConConBase
      // this->choose(chosen); // equivalent to this->choose = chosen


      // goal biased sampling: with 10% probability, sample from goal
      ...
      if (NULL != aConnected)
      {
        ...
        if (NULL != bConnected)
        {
          ...
          {
            // if a is tree[0], then end[0] = aConnected, otherwise end[0] = bConnected
